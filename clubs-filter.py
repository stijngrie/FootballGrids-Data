...
with open('Raw-Data-20242025\clubs.csv', mode='r', encoding='utf-8') as file:
    ...
    for row in new_rows:
        ...
        if (row['domestic_competition_id'] in ['GB1' , 'IT1', 'ES1', 'FR1', 'L1']):
            filtered_row = {key: value for key, value in row.items() if key not in ['stadium_seats', 'national_team_players', 'last_season', 'domestic_competition_id', 'stadium_name', 'foreigners_number', 'average_age', 'total_market_value', 'foreigners_percentage', 'coach_name', 'net_transfer_record', 'squad_size', 'filename']}
